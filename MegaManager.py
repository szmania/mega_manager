...
MEGA_ACCOUNTS_DATA = "MEGA_ACCOUNTS_DATA.txt"
MEGATOOLS_PATH = ''
MEGA_ACCOUNTS = ''
LOCAL_ROOT = ''
REMOTE_ROOT = ''
...
class MegaManager(object):
            ...
            for line in ins:
                if '=' in line:
                    ...
                    if line.startswith('MEGATOOLS_PATH'):
                        ...
                        global MEGA_ACCOUNTS_DATA
                        MEGA_ACCOUNTS_DATA = value
    ...
    def _dump_compressed_image_file_list(self):
        ...
        logger.debug(' Dumping self.__compressedImageFiles to %s file.' % COMPRESSED_IMAGES_FILE)

        compressedList_file = open('%s' % (COMPRESSED_IMAGES_FILE), 'w')

        for compressedFilePath in self.__compressedImageFiles:
    ...
    def _dump_compressed_video_fle_list(self):
        ...
        logger.debug(' Dumping self.__compressedVideoFiles to %s file.' % COMPRESSED_VIDEOS_FILE)

        compressedList_file = open('%s' % (COMPRESSED_VIDEOS_FILE), 'w')

        for compressedFilePath in self.__compressedVideoFiles:
            compressedList_file.write("%s\n" % compressedFilePath)
    ...
    def _create_thread_create_mega_accounts_data_file(self):
        """
        Create thread to create MEGA_ACCOUNTS_DATA file.
        """

        logger = getLogger('MegaManager._create_thread_create_mega_accounts_data_file')
        logger.setLevel(self.logLevel)

        logger.debug(' Creating thread to create "%s" file.' % MEGA_ACCOUNTS_DATA)

        t = Thread(target=self._create_mega_accounts_data_file, name='thread_createMEGA_ACCOUNTS_DATA_file')
    ...
    def _all_accounts_video_compression(self):
        ...
        foundUserPass = self._get_accounts_user_pass(file=MEGA_ACCOUNTS)
        ...
        while len(self.threads) > 0:
            ...
            if not time() - startTime > timeout:
                ...
                if not megaFileThreads_found and not megaFileFinished:
                    logger.info(' "%s" file creation complete!' % MEGA_ACCOUNTS_DATA)
    ...
    def _get_accounts_user_pass(self, file=MEGA_ACCOUNTS):
        """
                ...
                if remote_type == '0':
                    ...
                    if fileExt in extensions:
                        ...
                        if file_subPath is not '':
                            ...
                            if (path.exists(local_filePath)) and (local_filePath not in self.__compressedImageFiles) and (local_filePath not in self.unableToCompressImageFiles):
                                # timeout = 2
                                ...
                                if returnCode == 0:
                                    ...
                                    if path.exists(compressPath_backup):
                                        ...
                                        self.__compressedFiles = None
                                        self.__compressedFiles.append(local_filePath)
                ...
                if remote_type == '0':
                    ...
                    if fileExt in extensions:
                        ...
                        if file_subPath is not '':
                            ...
                            if (path.exists(local_filePath)) and (local_filePath not in self.__compressedVideoFiles) and (local_filePath not in self.unableToCompressVideoFiles):
                                # timeout = 2
                                ...
                                if returnCode == 0:
                                    ...
                                    self.__compressedVideoFiles.append(new_filePath)
    ...
    def _create_mega_accounts_data_file(self):
        """
        Create MEGA_ACCOUNTS_DATA file. File that has all fetched data of accounts and local and remote spaces of each account.
        ...
        logger.debug(' Creating MEGA_ACCOUNTS_DATA.txt file.')

        foundUserPass = self._get_accounts_user_pass()

        # copyfile(MEGA_ACCOUNTS_DATA, MEGA_ACCOUNTS_DATA + '.old')

        try:
            self.accounts_details_dict = {}
            with open(MEGA_ACCOUNTS_DATA, "w") as outs:
                for account in foundUserPass:
                    logger.debug(' Creating thread to gather details on account %s.' % account)

                    t_megaFile = Thread(target=self._get_account_details, args=(account['user'], account['pass'], REMOTE_ROOT),
            ...
            copyfile(MEGA_ACCOUNTS_DATA + '.old', MEGA_ACCOUNTS_DATA)
    ...
    def _get_account_details(self, username, password, remotePath=REMOTE_ROOT):
        """
        Creats dictionary of account data (remote size, local size, etc...) for MEGA_ACCOUNTS_DATA file.
    ...
    def _tear_down(self):
        ...
        if self.__compressedImageFiles:
            self._dump_compressed_image_file_list()

        if self.__compressedVideoFiles:
            self._dump_compressed_video_fle_list()
